...
class DropingController extends Controller
{
   ...
   public function index()
    {
        if(Auth::user()->role == 99)
            $dropings = Droping::where('status_2',1)->Where('id_droper',auth()->user()->id)->get();
    ...
    public function approvin($id)
    {
        ...
        if($droping->status == 3)
        {
                ...
                for($i=0;$i<$historydroping->count();$i++)
                {
                        ...
                        $kuedropingss = Kue::Where('id_user',$droping->id_user)->where('id_kue',$historydroping[$i] ->id_kue)->first();
                    if($kuedropingss != [])
                    {
                        $kuedropingss->update(['stock' => $kuedropingss->stock + $historydroping[$i]->stock]);
                        $kuedropingss->update(['return_masuk' => $kuedropingss->return_masuk + $historydroping[$i]->stock]);
                    }
                    else
                    {
                        $datasave = [
                ...
                for($i=0;$i<$historydroping->count();$i++)
                {
                    $kuedropingss = Kuedroping::Where('id_user',$droping->id_user)->where('id_kue',$historydroping[$i] ->id_kue)->first();
                    if($kuedropingss != [])
                    {
                        $kuedropingss->update(['stock' => $kuedropingss->stock + $historydroping[$i]->stock]);
                        $kuedropingss->update(['return_masuk' => $kuedropingss->return_masuk + $historydroping[$i]->stock]);
                        // $kuedropingss->update(['return' => $kuedropingss->return + $historydroping[$i]->return]);
                    }
                    else
                    {
                        $datasave = [
        ...
        else
        {
                        ...
                        if($historydroping->count() > 0)
                        {
                            for($i=0;$i<$historydroping->count();$i++)
                            {
                                ...
                                {
                                        $kuedropingss->update(['stock' => $kuedropingss->stock + $historydroping[$i]->stock]);
                                    $kuedropingss->update(['jumlah_droping' => $kuedropingss->jumlah_droping + $historydroping[$i]->jumlah_droping]);
   ...
   public function show($id)
    ...
    {   
        ...
        return view('droping.edit', compact('Droping','cookie','histori','kues'));
    }
    ...
    {
        $droping = Droping::where('id_droping',$id)->first();
        $droping->update(['status' => 9]);
        return back();
    }
